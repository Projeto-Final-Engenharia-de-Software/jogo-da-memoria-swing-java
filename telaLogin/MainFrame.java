...
public void initialize() {
    ...
    gbc.gridx = 5;
    gbc.gridy = 5;
    gbc.weightx = 1.0; // Reduzido para permitir espaço extra
    gbc.gridwidth = 2; // Aumenta o tamanho para ocupar 2 colunas
    gbc.insets.bottom = 10; // Distanciamento de 10 pixels na parte inferior

    // Defina uma largura mínima para o campo de texto
    gbc.ipadx = 100; // Ajuste o valor conforme necessário

    formPanel.add(lbLoginUsuario, gbc);

    gbc.gridy++;
    formPanel.add(tfLoginUsuario, gbc);

    gbc.gridy++;
    formPanel.add(lbSenhaUsuario, gbc);

    gbc.gridy++;
    formPanel.add(tfSenhaUsuario, gbc);
    ...
    btnLogin.addActionListener(new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (nickname.isEmpty() || senhaUsuario.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Usuário e/ou senha inválida!", "Aviso",
                        JOptionPane.ERROR_MESSAGE);
                return; // Parada no código
            } else {
                // Avança para a próxima tela
                JOptionPane.showMessageDialog(null, "Login reaizado com sucesso.", "Aviso", JOptionPane.INFORMATION_MESSAGE);
